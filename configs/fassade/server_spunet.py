...
mix_prob = 0.0
empty_cache = True
enable_amp = True # True == float16 statt float32
enable_wandb=False
...
# Gewichtung
class_weights = [0.5, 0.8, 5.0, 2.0, 1.5, 3.0]
...
data = dict(
    num_classes=6,
    ignore_index=-1,
    names=["Sonstiges", "Putz", "Beton", "Backstein", "Dachziegel", "Stein"],
    train=dict(
        type=dataset_type,
        split="train",
        data_root=data_root,
        transform=[
            ...
            dict(type="ToTensor"),
            dict(type="Collect", keys=("coord", "grid_coord", "segment"), feat_keys=("color", "normal", "extra_features")),
        ],
        test_mode=False,
        loop=10, 
    ),
